 ...
 */
public class MonthlyGameListFragment extends RetryRequestFragment<UiGameRecordListBinding> implements BGARefreshLayout.BGARefreshLayoutDelegate {

    private static final String TAG = "PersonalCenter.MonthlyGameListFragment";
    ...
    @Override
    public void init() {
        mAdapter = new GamePerformanceAdapter(getActivity().getLayoutInflater(), getString(R.string.personal_center_month_game));
        ...
        mBinding.dataRv.setLayoutManager(new LinearLayoutManager(this.getContext()));

        mBinding.dataRv.setEmptyView(mBinding.emptyView);
        mBinding.emptyView.setText("您还没报名参加月赛");
        mBinding.dataRv.setOnItemClickListener(new VRecyclerView.OnItemClickListener() {
            @Override
            public void onItemClick(RecyclerView parent, View view, int position, long id) {
                Bundle bundle = new Bundle();
                bundle.putInt(ProtocolConstants.IntentParams.GAME_ID, mAdapter.getItem(position).getId());
                bundle.putParcelable(ProtocolConstants.IntentParams.GAME_INFO, mAdapter.getItem(position));
                PluginStubBus.doAction(getActivity(), PluginConstants.Plugin.PLUGIN_NAME_P_GAMES, PluginConstants.Games.Action.CMD_START_GAME_DETAIL, 0, bundle);
            }
        });
    ...
    private void updateGameList() {
        ...
        /**
         * 赛事类型里面没有专门的月赛，城市赛属于月赛，所以这里暂时填写city。
         */
        netScene = new NetSceneGetAccountGame(ProtocolConstants.GameType.CITY_GAME, mOffset,
            ...
            public void onResponse(IRequest request, Racecar.GetAccountGameResponse result) {
                ...
                mBinding.emptyView.setText(R.string.personal_no_monthly_game);
